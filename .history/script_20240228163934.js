...
async function fetchYoutubeTrailer(gameName) {
  try {
    const response = await fetch(
      ...
      console.log(  `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&q=${gameName}trailer&key=AIzaSyCsEU3Fe6wNACeFTvZQgKA46QnreQL12NI`)
    );
    if (!response.ok) {
  ...
  gameData.forEach(async (game) => {
    ...
    if (game.stores !== null) {
      ...
      async function getTrailer() {
        ...
        if (trailerTitle.includes(game.name)) {
          console.log("IT DOES");
