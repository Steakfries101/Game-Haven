...
async function loopData(gameName) {
  ...
  searchDisplay(gameData, gameName);
...
function searchDisplay(data, gameName) {
  const msg = document.createElement("h2");

  if (data == "") {
    msg.textContent = `No game by the name of ${gameName}`;
  } else {
    msg.textContent = `Showing results for ${gameName}`;
  }
